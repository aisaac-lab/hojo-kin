      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            url: {
              type: 'string',
              description: 'スクレイピング開始URL（デフォルト: 東京都の補助金一覧）',
            },
            saveToDb: {
              type: 'boolean',
  ...
  try {
    switch (name) {
      ...
      case 'scrape_smart_hojokin': {
        const url = (args?.url as string) || 'https://www.smart-hojokin.jp/subsidy/prefectures/13/areas/1110?exclude=true&page=1';
            ...
            for (const subsidy of subsidies) {
              try {
                await prisma.subsidy.create({
                  data: {
                    jgrantsId: `SCRAPED-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
                    title: subsidy.title,
