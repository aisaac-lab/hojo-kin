...
describe('FileStoreService', () => {
	...
	beforeEach(async () => {
		...
		testSubsidy = await db.subsidy.create({
			data: {
				jgrantsId: 'TEST-001',
				title: 'テスト補助金',
	...
	describe('readSubsidyMarkdown', () => {
		...
		it('should return null for non-existent file', async () => {
			const content = await fileStoreService.readSubsidyMarkdown(
				'NON-EXISTENT'
			);
	...
	describe('batchCreateFiles', () => {
		it('should create multiple files', async () => {
			const subsidy2 = await db.subsidy.create({
				data: {
					jgrantsId: 'TEST-002',
					title: 'テスト補助金2',
